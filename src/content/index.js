export default {
  nav: {
    logo: 'MaiJsp',
    links: [
      { text: 'About', to: 'mywork' },
      { text: 'Contact', to: 'mycontact' },
    ],
  },
  header: {
    img: process.env.PUBLIC_URL + '/assets/mypic.jpg',
    imgPlaceholder:
      'data:image/jpeg;base64,',
    text: ["Hi! I'm Mai", "<Saranphon Phaithoon/>", 'Software Engineering student'],
    typical: [
      'Software Engineering 🖥',
      2000,
      'Music Lover 🎹',
      2000,
      'Travelling ✈️',
      2000,
    ],
    btnText: 'Discover More',
  },

  stack: {
    title: 'Stack',
    tech: [
      {
        img: process.env.PUBLIC_URL + '/assets/mongo.png',
        alt: 'mongodb',
      },
      {
        img: process.env.PUBLIC_URL + '/logo512.png',
        alt: 'react',
      },
      {
        img: process.env.PUBLIC_URL + '/assets/express.png',
        alt: 'express',
      },
      {
        img: process.env.PUBLIC_URL + '/assets/node.png',
        alt: 'node',
      },
    ],
    desc: `Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only`,
  },

  project: {
    title: "PROJECTS",
    projects: [
      {
        name: "FIXME bot",
        description: "A GitHub bot that can identify self-admitted technical debt in source code comments and inform the developers to fix their unfinished work",
        tech: ["React", "MongoDB", "Flask", "Python"],
        link: ""
      },
      {
        name: "QC Management System",
        description: "A web application developed for QC department to track the information of their product, visulize production line for QC managers",
        tech: ["React", "Firebase"],
        link: "https://github.com/maijsp/wong-wai-vit"
      },
      {
        name: "Gemini Observatory System",
        description: "A web application that allow science observers record information of star systems and used to control observatory",
        tech: ["Springboot", "MySQL"],
        link: "https://github.com/maijsp/GeminiFive"
      },
      {
        name: "Penquiz: Android Quiz Application",
        description: "A simple online quiz application that users can practice their knowledge about computer network topics e.g. TCP/IP",
        tech: ["Kotlin", "Firebase"],
        link: "https://github.com/maijsp/penquiz"
      },
      {
        name: "Simple Java Search Engine",
        description: "A simple search engine that can rank relevant documents from search query using a numerical statistic, TF-IDF",
        tech: ["Java"],
        link: "https://github.com/maijsp/penquiz"
      },
      {
        name: "Web Programming Project",
        description: "An informative website about ethic groups in Thailand integrated with web accessibilities i.e. font size, color blindness, and Text To Speech (TTS)",
        tech: ["HTML5", "CSS3", "JavaScript", "Bootstrap"],
        link: ""
      }
    ]
  }
};
